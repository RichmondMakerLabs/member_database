...
if ($_SERVER["REQUEST_METHOD"] == "POST")       // i.e. if called with some parameters
{
    ...
    if (($known_as) && ($first_name) && ($last_name) && ($contact >1))   {
        ...
        if (!$result) {
            $display = " Caught error " . $mysqli->error . "<br>\n";
            $display .= "<a href='addmember.html'>Click here</a> to return to the form page";
        } else {
            $display0 = "$known_as ... $first_name $last_name";
            $display = "That went well, <a href='rml.php'>click here</a> to return to the front page";
        }
    } else {
        $display = " Something's missing. <br>We need first & last names and at least two contact details <br>";
        $display .= "<a href='addmember.html'>Click here</a> to start again";
     }
}
